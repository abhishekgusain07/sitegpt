...
const FAQ = () => {
  ...
  // Calculate the middle index to split the FAQs into two columns
  const middleIndex = Math.ceil(filteredFAQs.length / 2);
  ...
  return (
    <section className="bg-white py-12 sm:py-16 lg:py-20">
      <div className="max-w-screen-xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* Modified FAQ grid */}
        <div className="grid grid-cols-1 gap-x-16 gap-y-5 mt-8 lg:grid-cols-2 lg:items-start">
          <div className="space-y-5">
            {filteredFAQs.slice(0, middleIndex).map((faq, index) => (
              <FAQItem key={index} question={faq.question} answer={faq.answer} />
            ))}
          </div>
          <div className="space-y-5">
            {filteredFAQs.slice(middleIndex).map((faq, index) => (
              <FAQItem key={index} question={faq.question} answer={faq.answer} />
            ))}
          </div>
        </div>
...
const FAQItem = ({ question, answer }:{
  ...
  return (
    <div className="bg-white">
      ...
      >
        <summary className="flex cursor-pointer items-center justify-between text-lg font-medium text-gray-900 py-2">
          {question}
        ...
        <div className={`mt-3 pr-12 ${isOpen ? 'block' : 'hidden'}`}>
